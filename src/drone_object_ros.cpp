...
void DroneObjectROS::initROSVars(ros::NodeHandle &node) {
  ...
  isPosctrl = false;
  isVelMode = false;
  pubTakeOff = node.advertise<std_msgs::Empty>("/sjtu_drone/takeoff", 1024);
  pubLand = node.advertise<std_msgs::Empty>("/sjtu_drone/land", 1024);
  pubReset = node.advertise<std_msgs::Empty>("/sjtu_drone/reset", 1024);
  pubPosCtrl = node.advertise<std_msgs::Bool>("/sjtu_drone/posctrl", 1024);
  pubCmd = node.advertise<geometry_msgs::Twist>("/sjtu_drone/cmd_vel", 1024);
  pubVelMode = node.advertise<std_msgs::Bool>("/sjtu_drone/vel_mode", 1024);
}
